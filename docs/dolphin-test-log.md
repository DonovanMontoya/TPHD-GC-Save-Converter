...
## 2026-08-14 — Forward location-struct exclusion (re-validated)
...
```text
Date: 2026-08-14
Dolphin: 2606a, isolated profile, Null video backend, muted
Game image: Legend of Zelda, The - Twilight Princess (USA).iso, disc ID GZ2E01
Game SHA-256: 490ef919f413e00daedb4711777c3be05ef9afc12c7acca7675c721c9393c814
       — byte-identical to the image used for the superseded 2026-08-11 run,
       so the two observations are directly comparable
Input GCI: exported-progress-paired-wood-scent.gci (sha256 57a8b58a…), slot 0
       duplicated into all three quest logs so the timed script's selection is
       deterministic
Input script: tests/dolphin/load-slot-2-first-run.txt
Assertion: --expect-disc-path '/F_SP121/' (delimited segment, not a substring)
Runtime: 85.0s, terminated by the harness timeout as designed

Expected stage/room: F_SP121, point 1, room 2
Observed stage/room: PASS — 'Expected disc path observed'. The file monitor
       recorded res/Stage/F_SP121/STG_00.arc with rooms R01_00, R02_00, and
       R03_00. R02_00.arc matches the expected room 2.
Observed boot order: res/Stage/F_SP102/ (title/attract demo) loads first at
       disc-access 38-46, then F_SP121 at 66-75 once the save is loaded.
Observed gameplay progression: after the stage, Alink.arc, AlAnm.arc,
       Midna.arc, an enemy REL (d_a_e_yd_leaf.rel), and background archives
       load, so the room reached live actor loading rather than stalling in a
       menu.
Conclusion: the regenerated bytes remain loader-compatible and reach the same
       return place as the superseded 2026-08-11 output.
```

Evidence: `artifacts/dolphin-smoke/paired-relocation-2026-08-14/disc-paths.log`
(local, git-ignored), 123 recorded disc accesses.

Scope limit, per the evidence rules in `AGENTS.md`: this is a stage/room load
observation. The run used the Null video backend with no screenshot, so rupee
count, children scent, Sense, and Midna riding were **not** visually confirmed
this pass; they are present in the input bytes and were visually observed on the
superseded 2026-08-11 build. This run proves the location-struct exclusion did
not break loading or change the return place — it does not independently
re-verify gameplay semantics.
